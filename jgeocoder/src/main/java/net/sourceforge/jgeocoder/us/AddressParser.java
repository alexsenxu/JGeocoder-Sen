...
public class AddressParser{
  ...
  public static Map<AddressComponent, String> parseAddress(String rawAddr, boolean autoCorrectStateSpelling){
    ...
    if(m.matches()){
      ...
      //System.out.println("ret.get(AddressComponent.TLID) is: "+ret.get(AddressComponent.TLID));
      //this line caused nullpointer error
  ...
  private static String designatorConfusingCitiesCorrection(Map<AddressComponent, String> parsedLocation, String input){
      ...
      //Sen mod, for Java nullpointer expection found in dealing with IntellData
        if (parsedLocation.isEmpty()||parsedLocation==null) return input;
        //System.out.println("input: "+input);
        //System.out.println("parsedLocation : "+parsedLocation.toString());
    //
	  ...
	  Matcher m = STREET_DESIGNATOR_CHECK.matcher(street);//Sen doc: first check if there is street type designator in the street field, if there is, proceed
	  if(m.find()){
		  String parsedstate = parsedLocation.get(AddressComponent.STATE);
		  //System.out.println(parsedstate);
                  if(parsedstate == null){//if parsedstate is empty, then try to parse state from the city field
		  ...
		  if ((normalizedState != null)
                          //Sen mod
                          && (SpecialData.C_MAP.keySet().contains(normalizedState))){//if normalizedState is a valid US State
		  ...
		  for(String state : stateSet){
                      //Sen mod, null pointer error when parsing "1227 E LUDLOW RIDGE RD PORT LU, 98365"
//                      System.out.println(state);
//                      System.out.println(stateSet.toString());
                      //System.out.println(stateSet.toString());
                      //Found that there are cases where STATE being parsed is not actually a state
  ...
  public static void main(String[] args){
      ...
      Map<AddressComponent, String> mc=AddressParser.parseAddress(fulladdress,false);
