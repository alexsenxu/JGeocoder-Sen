...
public class JGeocoder{
  ...
  public JGeocodeAddress geocodeAddress(String addrLine){
    ...
    try {
      ...
      //Sen mod: This DatabaseException is causing hadoop job to fail. 
      throw new RuntimeException("Unable to query tiger/line database "+e.getMessage());
    }
    if(hit != null){
      acuracy = GeocodeAcuracy.STREET;
      
      //Sen mod: validate the streetNum, Integer.valueOf(CommonUtils.nvl(toAddl, "99999"))
      Geo geo = Geocoder.geocodeFromHit(Integer.parseInt(CommonUtils.nvl(Geocoder.validate(hit.streetNum),"0")), hit);
  ...
  public JGeocoder(JGeocoderConfig config){
    ...
    try {
        //Sen mod, changed the first boolean (second parameter) to true
      _zipDb.init(new File(config.getJgeocoderDataHome()), true, false);
  ...
  public static void main(String[] args) {
      ...
      JGeocodeAddress jgAddr=jg.geocodeAddress("320 Vairo blvd, State College, PA 16803");
      System.out.println(ToStringBuilder.reflectionToString(jgAddr, ToStringStyle.MULTI_LINE_STYLE));
  }
