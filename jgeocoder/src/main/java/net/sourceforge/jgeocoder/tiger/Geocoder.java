...
class Geocoder {
  ...
  static Geo geocodeFromHit(int streetnum, TigerLineHit hit){
      //Sen mod
      //check if any of the Interger.valueOf are null, if null, replace with sensible values
      String frAddr=validate(hit.frAddR);
      String frAddl=validate(hit.frAddL);
      String toAddr=validate(hit.toAddR);
      String toAddl=validate(hit.toAddL);
      
      
      //System.out.println(streetnum +", "+ hit.toString());
      return geocode(streetnum, hit.tlid, Integer.valueOf(CommonUtils.nvl(frAddr,"0")), Integer.valueOf(CommonUtils.nvl(frAddl,"0")), 
        Integer.valueOf(CommonUtils.nvl(toAddr,"99999")), Integer.valueOf(CommonUtils.nvl(toAddl, "99999")), 
        Integer.valueOf(CommonUtils.nvl(hit.zipL,"00000")), Integer.valueOf(CommonUtils.nvl(hit.zipR,"00000")), hit.toLat, hit.toLon, hit.frLon, hit.frLat, 
...
//        hit.lon6, hit.lat6, hit.lon7, hit.lat7, hit.lon8, hit.lat8, hit.lon9, hit.lat9, hit.lon10, hit.lat10);
  ...
  //Sen mod: in the loaded spatial database, sometime the fraddr and toaddr are not valid "350N2454", this step validate the addr
  public static String validate(String AddR) {
        ...
        if ((AddR!=null)&&(AddR!="")){
            ...
            //if there are letters, return the numbers before the letter or ""
            if (addR.matches(".*[A-Za-z]{1,}.*")){
                //System.out.println("there is letters in the Addr field:"+ret);
                String ret=addR.split("[A-Za-z]")[0];//if there is a non digit letter in the AddR field, return the numbers before the word.
                if (ret.isEmpty()) ret=addR.split("[A-Za-z]")[1];//if the AddR field begins with letter, use the second split
                if (ret.isEmpty()) ret=null;//if the ret is still empty, return null
                return ret;
            }else{
                //if only number, check if it is too large (more than 9 digits)
                if (addR.length()>8)  {
                    return null;
                }else{
                return addR;
...
//    ---If no, trim the ratio so that it applies only to this segment and return calculated point.
  ...
  public static void main(String[] args){
      System.out.println(validate("3491946686"));
      System.out.println(Integer.valueOf(CommonUtils.nvl(validate("3491946686"),"0")));
  }
